...
def fix_years_of_experience(blurb: str, name: str, correct_years: int) -> str:
    ...
    # If blurb is empty, return a default message
    if not blurb or not name:
        return f"{name} is a professional with {correct_years} years of experience."
    
    # Split into paragraphs first
    paragraphs = blurb.split('\n\n')
    if len(paragraphs) < 2:
        # If we don't have clear paragraphs, try to split on double spaces or other common paragraph breaks
        paragraphs = re.split(r'\n{2,}|\s{2,}', blurb)
    
    # Ensure we have at least two paragraphs
    ...
    # Remove year mentions from all but first sentence of first paragraph
    year_patterns = [
        r'\b(\d+|one|two|three|four|five|six|seven|eight|nine|ten|fifteen|twenty)[\s-]years?\b',
        r'\bover (\d+|a|one|two|three|four|five|six|seven|eight|nine|ten|fifteen|twenty)[\s-]decades?\b',
        r'\b(\d+|one|two|three|four|five|six|seven|eight|nine|ten|fifteen|twenty)[\s-]decades?\b'
    ]
    
    # Clean years from remaining sentences
    remaining_sentences = []
    for sentence in sentences[1:]:
        ...
        if cleaned.strip():
            remaining_sentences.append(cleaned.strip())
    
    # Create standardized first sentence
    profession_match = re.search(r'(?:is|as) an? ([A-Za-z\s-]+)', sentences[0], re.IGNORECASE)
    profession = profession_match.group(1).strip() if profession_match else "professional"
    new_first_sentence = f"{name} is a highly experienced {profession} with {correct_years} years of experience in the industry."
    
    # Reconstruct first paragraph
    first_para = new_first_sentence
    if remaining_sentences:
        first_para += " " + " ".join(remaining_sentences)
    
    # Clean years from second paragraph
    second_para = paragraphs[1]
    for pattern in year_patterns:
        second_para = re.sub(pattern, '', second_para, flags=re.IGNORECASE)
